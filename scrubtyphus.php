...
/**
 * scrubtyphus.php
 * แจ้งเตือนผู้ป่วยสครับไทฟัส — HR-CENTER 4.0
 * — Query ตรงจาก HOSxP (vn_stat + lab_order + patient)
 * — ส่ง LINE Flex ผ่าน MOPH Alert ด้วย scrub_action.php
 */
...
try {
  ...
  $rows = scrub_source_rows($start, $end);
} catch (Throwable $e) {
...
// ── Page variables ────────────────────────────────────────────────────────────
$PAGE_TITLE = 'สครับไทฟัส (Scrub Typhus)';
$PAGE_KEY   = 'scrub';
$EXTRA_HEAD = '
...
<div class="page-header">
  <h1>
    <span class="msi me-2" style="color:#059669">bug_report</span>
    <?= htmlspecialchars($PAGE_TITLE) ?>
  </h1>
  <div class="d-flex gap-2 flex-wrap">
    <span class="badge rounded-pill"
          style="background:#ecfdf5;color:#064e3b;border:1px solid #a7f3d0;font-size:.78rem;padding:.35rem .75rem">
...
<div class="row g-3 mb-3">
  <div class="col-6 col-lg-4">
    <div class="kpi-card">
      <div class="kpi-icon bg-green"><span class="msi">bug_report</span></div>
      <div>
        <p class="kpi-label">ทั้งหมด (<?= date('d/m',strtotime($start))?>–<?= date('d/m',strtotime($end))?>)</p>
        <p class="kpi-value" style="color:#059669"><?= number_format($kpi['total']) ?></p>
      </div>
    </div>
  </div>
  <div class="col-6 col-lg-4">
  ...
  <form class="row g-2 align-items-end" method="get">
    ...
    <div class="col-sm-6 col-md-3 d-flex gap-2 align-items-end">
      <button class="btn btn-sm flex-grow-1"
              style="background:linear-gradient(135deg,#34d399,#059669);color:#fff;border:none;border-radius:8px">
...
<div class="card p-3">
  <div class="table-responsive">
    <table id="tblScrub" class="table table-hover align-middle nowrap" style="width:100%">
      <thead>
        <tr>
          <th>#</th>
          <th>VN</th>
      ...
      ?>
        <tr>
          ...
          <td>
            ...
            <?php if ($icd10): ?>
              <span class="badge rounded-pill mt-1"
                    style="background:#ecfdf5;color:#064e3b;border:1px solid #a7f3d0;font-size:.75rem">
                <?= $icd10 ?>
              </span>
            <?php endif; ?>
          </td>
          <td>
            <span class="badge rounded-pill"
                  style="background:#dcfce7;color:#166534;font-size:.82rem;padding:.3rem .7rem;font-weight:600">
              <?= $result ?: '-' ?>
            </span>
          </td>
          <td>
  ...
  $("#tblScrub").DataTable({
    ...
    buttons: [
      {extend:"excel", text:\'<span class="msi me-1">table_view</span> Excel\',
       ...
       title:"scrub_alert_"+new Date().toLocaleDateString("th-TH")},
      ...
      .then(json => {
        if (json.ok) {
          ...
          Swal.fire({icon:"error",title:"ส่งไม่สำเร็จ",
            text:json.msg??"เกิดข้อผิดพลาด",confirmButtonColor:"#059669"});
        }
      })
      .catch(err => {
        btn.disabled = false;
