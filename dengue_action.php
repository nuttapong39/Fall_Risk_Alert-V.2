...
/**
 * dengue_action.php
 * AJAX action handler สำหรับ dengue.php
 *
 * POST action=send  vn=<VN>
 *   → query HOSxP ด้วย VN + lab_items_code '2891'
...
// ── Constants ─────────────────────────────────────────────────────────────────
define('DENGUE_TYPE',     'dengue');

if (!defined('MOPH_TIMEOUT'))   define('MOPH_TIMEOUT',   30);
if (!defined('DENGUE_CLIENT_KEY'))
  define('DENGUE_CLIENT_KEY', defined('MOPH_CLIENT_KEY') ? MOPH_CLIENT_KEY : '');
if (!defined('DENGUE_SECRET_KEY'))
  define('DENGUE_SECRET_KEY', defined('MOPH_SECRET_KEY') ? MOPH_SECRET_KEY : '');
...
function dengue_log(array $row, $code, $resp, $err = null): void {
  global $LOG_FILE;
...
try {
  ...
  $row = dengue_source_by_vn($vn);

  if (!$row) {
    echo json_encode(['ok'=>false,
      'msg'=>"ไม่พบข้อมูล VN: {$vn} (lab_items_code=".(module_filter('dengue')['lab_code'] ?? '2891').")"]);
    exit;
  }
  ...
  // ── Build Flex payload ────────────────────────────────────────────────────
  $payload = buildDiseasePayload($row, DENGUE_TYPE);
  $body    = json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);

  if ($body === false) {
    $jsonErr = json_last_error_msg();
    dengue_log($row, 0, null, "JSON_ENCODE_FAIL: $jsonErr");
