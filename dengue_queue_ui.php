...
<style>
  ...
  /* ── Floating action bar ── */
  #dqBar{position:fixed;left:50%;transform:translateX(-50%) translateY(120%);bottom:18px;z-index:1050;
  ...
  #dqBar.show{transform:translateX(-50%) translateY(0)}
  #dqBar .btn{border-radius:999px;font-size:.82rem}
...
<form method="post" action="dengue_queue_action.php" id="dqActionForm">
  ...
  <input type="hidden" name="action" id="dqAction" value="">
    ...
    <div class="table-responsive">
      <table id="tblDengueQ" class="table table-hover align-middle nowrap" style="width:100%">
        <thead>
          <tr>
            <th style="width:30px">
              <input type="checkbox" class="form-check-input" id="dqAll" aria-label="เลือกทั้งหมด">
            </th>
            <th>ID</th>
            <th>สถานะ</th>
            <th>VN</th>
        ...
        ?>
          <tr>
            <td><input type="checkbox" class="form-check-input dqchk" name="ids[]" value="<?= $r['id'] ?>"></td>
...
<div id="dqBar">
  ...
  <span id="dqCount">0 รายการที่เลือก</span>
  <button type="button" class="btn btn-sm" style="background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;border:none"
  ...
  <button type="button" class="btn btn-outline-light btn-sm" id="dqCancel"><span class="msi">close</span></button>
</div>
...
$(function(){
  ...
  /* ── Checkbox all ── */
  const $dqAll = $("#dqAll");
  $dqAll.on("change", function(){
    $("#tblDengueQ tbody .dqchk").prop("checked", this.checked);
    updateCount();
  });
  $(document).on("change", ".dqchk", updateCount);
  table.on("draw", function(){ $dqAll.prop("checked", false); updateCount(); });
  function updateCount(){
    const n = $("#tblDengueQ tbody .dqchk:checked").length;
    $("#dqCount").text(n + " รายการที่เลือก");
    $("#dqBar").toggleClass("show", n > 0);
  }

  $("#dqCancel").on("click", function(){
    $(".dqchk, #dqAll").prop("checked", false);
    updateCount();
  });

  /* ── Bulk actions ── ตรวจจาก dengue_queue_action.php:
  ...
  $("#dqBar [data-act]").on("click", function(){
    ...
    Swal.fire({
      ...
      html: dqDescs[action] + " สำหรับ " + n + " รายการที่เลือก (เฉพาะแถวในหน้าปัจจุบัน)",
      showCancelButton:true, confirmButtonText:"ยืนยัน",
      cancelButtonText:"ยกเลิก", reverseButtons:true, focusCancel:true,
      confirmButtonColor:"#d97706"
    }).then(r => {
      if (!r.isConfirmed) return;
      document.getElementById("dqAction").value = action;
      document.getElementById("dqActionForm").submit();
    });
  });
