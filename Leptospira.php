...
<style>
  ...
  /* ── Floating action bar ── */
  #lepBar{position:fixed;left:50%;transform:translateX(-50%) translateY(120%);bottom:18px;z-index:1050;
  ...
  #lepBar.show{transform:translateX(-50%) translateY(0)}
  #lepBar .btn{border-radius:999px;font-size:.82rem}
...
<!-- ── Table + Bulk Actions ───────────────────────────────────── -->
<form method="post" action="lepto_queue_action.php" id="lepForm">
  <input type="hidden" name="token" value="<?= htmlspecialchars(LEPTO_Q_UI_TOKEN) ?>">
  <input type="hidden" name="action" id="lepAction" value="">
    ...
    <div class="table-responsive">
      <table id="tblDengueQ" class="table table-hover align-middle nowrap" style="width:100%">
        <thead>
          <tr>
            <th style="width:30px">
              <input type="checkbox" class="form-check-input" id="lepAll" aria-label="เลือกทั้งหมด">
            </th>
            <th>ID</th>
            <th>สถานะ</th>
            <th>VN</th>
        ...
        ?>
          <tr>
            <td><input type="checkbox" class="form-check-input lepchk" name="ids[]" value="<?= $r['id'] ?>"></td>
...
<div id="lepBar">
  ...
  <span id="lepCount">0 รายการที่เลือก</span>
  <button type="button" class="btn btn-sm" style="background:linear-gradient(135deg,#14B8A6,#0F766E);color:#fff;border:none"
  ...
  <button type="button" class="btn btn-outline-light btn-sm" id="lepCancel"><span class="msi">close</span></button>
</div>
...
$(function(){
  ...
  /* ── Checkbox all ── */
  const $lepAll = $("#lepAll");
  $lepAll.on("change", function(){
    $("#tblDengueQ tbody .lepchk").prop("checked", this.checked);
    updateCount();
  });
  $(document).on("change", ".lepchk", updateCount);
  table.on("draw", function(){ $lepAll.prop("checked", false); updateCount(); });
  function updateCount(){
    const n = $("#tblDengueQ tbody .lepchk:checked").length;
    $("#lepCount").text(n + " รายการที่เลือก");
    $("#lepBar").toggleClass("show", n > 0);
  }

  $("#lepCancel").on("click", function(){
    $(".lepchk, #lepAll").prop("checked", false);
    updateCount();
  });

  /* ── Bulk actions ── ตรวจจาก lepto_queue_action.php:
  ...
  $("#lepBar [data-act]").on("click", function(){
    ...
    Swal.fire({
      ...
      html: lepDescs[action] + " สำหรับ " + n + " รายการที่เลือก (เฉพาะแถวในหน้าปัจจุบัน)",
      showCancelButton:true, confirmButtonText:"ยืนยัน",
      cancelButtonText:"ยกเลิก", reverseButtons:true, focusCancel:true,
      confirmButtonColor:"#0F766E"
    }).then(r => {
      if (!r.isConfirmed) return;
      document.getElementById("lepAction").value = action;
      document.getElementById("lepForm").submit();
    });
  });
