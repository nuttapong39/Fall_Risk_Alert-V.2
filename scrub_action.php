...
/**
 * scrub_action.php
 * AJAX action handler สำหรับ scrubtyphus.php
 *
 * POST action=send  vn=<VN>
 *   → query HOSxP ด้วย VN + lab_items_code '291'
...
if (!defined('SCRUB_CLIENT_KEY'))
  define('SCRUB_CLIENT_KEY', defined('MOPH_CLIENT_KEY') ? MOPH_CLIENT_KEY : '');
if (!defined('SCRUB_SECRET_KEY'))
  define('SCRUB_SECRET_KEY', defined('MOPH_SECRET_KEY') ? MOPH_SECRET_KEY : '');
...
function scrub_log(array $row, $code, $resp, $err = null): void {
  global $LOG_FILE;
...
try {
  ...
  // ── Build Flex payload ────────────────────────────────────────────────────
  $payload = buildDiseasePayload($row, SCRUB_TYPE);
  $body    = json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);

  if ($body === false) {
    $jsonErr = json_last_error_msg();
    scrub_log($row, 0, null, "JSON_ENCODE_FAIL: $jsonErr");
