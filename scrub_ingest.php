<?php
/**
 * scrub_ingest.php — CLI worker สำหรับ Task Scheduler
 * ─────────────────────────────────────────────────────────────────────────────
 * ต้องรันผ่าน PHP CLI เท่านั้น (ไม่ใช่ผ่านเว็บ)
 *
 * Usage:
 *   php scrub_ingest.php                        — ดึง + ส่ง 7 วันล่าสุด (default)
 *   php scrub_ingest.php dryrun                 — แสดงผลเฉพาะ ไม่ส่งจริง
 *   php scrub_ingest.php start 2025-06-01       — backfill ตั้งแต่วันที่ระบุ
 *   php scrub_ingest.php start 2025-06-01 end 2025-12-31
 */

/* ── CLI guard ─────────────────────────────────────────────────── */
if (PHP_SAPI !== 'cli') {
  http_response_code(403);
  exit("scrub_ingest.php must be run from CLI only.\n");
}
...
$LOG_FILE = $LOG_DIR . '/scrub_task_run.log';
$MOPH_LOG = $LOG_DIR . '/moph_alert_scrub.log';
...
/* ═══ STEP 2: Upsert → scrub_queue ═════════════════════════════ */
if ($isDry) {
  dng_log('DRYRUN — ไม่ upsert จริง แสดงเฉพาะผลลัพธ์:');
  foreach ($hosxpRows as $r) {
    $r = row_to_utf8($r);
    dng_log("  ROW vn={$r['vn']} hn={$r['hn']} vstdate={$r['vstdate']} result={$r['result']}");
  }
  dng_log('=== scrub_ingest DONE [DRYRUN] ===');
  exit(0);
}
...
foreach ($pending as $qRow) {
  ...
  if (!$err && ($code >= 200 && $code < 300) && $ok) {
    ...
    telegram_mirror('scrub', '🦠 แจ้งเตือนผู้ป่วยสครับไทฟัส', $qRow);
