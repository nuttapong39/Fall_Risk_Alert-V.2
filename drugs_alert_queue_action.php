<?php
/**
 * drugs_alert_queue_action.php — endpoint ของหน้า drugs_alert.php
 *   action=import_hosxp : AJAX (JSON) — sync จาก HOSxP เข้า drugs_alert_queue (recheck-only, status=1)
 *   action=send_now     : ส่งซ้ำทันที (bypass cooldown, bypass เงื่อนไข icode ปัจจุบัน — เลือกเองแล้ว)
 *   action=requeue      : รีเซ็ต status=0 attempt=0
 *   action=clear_error  : ล้างข้อความ error
 * 3 ตัวหลังต้องมี token (DRUGS_ALERT_UI_ACTION_TOKEN) + ids[] แล้ว redirect กลับ UI พร้อม ?msg=
 */
...
require_once __DIR__ . '/flex_builders.php';   // buildDrugsAlertPayload()
...
if ($action === 'import_hosxp') {
  ...
  try {
    ...
    // status=1 ทันที — Sync นี้เป็น recheck เท่านั้น ไม่ให้ Worker ส่ง LINE (ตรงกับทุก module)
    $ins = $dbcon->prepare(
      "INSERT INTO drugs_alert_queue
       ...
       VALUES (:hn,:fn,:cid,:tel,:age,:sex,:addr,:code,:dn,:st,:un,:vd,:qty,:price, 1)
    ...
    $exist = $dbcon->prepare("SELECT id FROM drugs_alert_queue WHERE hn=? AND icode=? AND vstdate=?");
...
if (!$ids) { header('Location: drugs_alert.php?msg=no_ids'); exit; }
...
function drugs_alert_log_send($row, $code, $resp, $err = null) {
  global $SEND_LOG;
...
/* ══ ส่ง 1 รายการ — ปุ่ม "ส่งซ้ำทันที" แบบเลือกเอง ไม่กรอง icode ปัจจุบัน (ผู้ใช้เลือกแถวเองแล้ว) ══ */
function drugs_alert_send_one(PDO $db, int $id): array {
  $get = $db->prepare("SELECT * FROM drugs_alert_queue WHERE id=:id");
  $get->execute([':id' => $id]);
  ...
  if ($body === false) {
    ...
    drugs_alert_log_send($row, 0, null, "JSON_ENCODE_FAIL: $e");
    $db->prepare("UPDATE drugs_alert_queue SET last_attempt_at=NOW(), attempt=attempt+1, last_error=:e WHERE id=:id")
  ...
  drugs_alert_log_send($row, $code, $resp, $err ?: null);
  ...
  if ($code >= 200 && $code < 300 && $ok) {
    ...
    $db->prepare("UPDATE drugs_alert_queue
...
try {
  ...
  if ($action === 'requeue') {
    $st = $dbcon->prepare("UPDATE drugs_alert_queue
                           SET status=0, attempt=0, last_attempt_at=NULL, last_error=NULL,
                               out_ref=NULL, line_message_id=NULL, sent_at=NULL
                           WHERE id IN ($place)");
    $st->execute($ids);
    header('Location: drugs_alert.php?msg=requeued&affected=' . $st->rowCount()); exit;

  } elseif ($action === 'clear_error') {
    $st = $dbcon->prepare("UPDATE drugs_alert_queue SET last_error=NULL WHERE id IN ($place)");
    $st->execute($ids);
    header('Location: drugs_alert.php?msg=cleared&affected=' . $st->rowCount()); exit;

  } elseif ($action === 'send_now') {
    $ok = 0; $fail = 0;
    foreach ($ids as $id) {
      [$o] = drugs_alert_send_one($dbcon, (int)$id);
      $o ? $ok++ : $fail++;
      usleep(random_int(10, 80) * 1000);
    }
    header('Location: drugs_alert.php?msg=sendnow&ok=' . $ok . '&fail=' . $fail); exit;

  } else {
    header('Location: drugs_alert.php?msg=bad_action'); exit;
  }
} catch (Throwable $e) {
  header('Location: drugs_alert.php?msg=err&detail=' . urlencode($e->getMessage())); exit;
}
