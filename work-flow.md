...
- **Auto-send ตัดสินรายแถวด้วย `created_at` — ไม่ใช่ flush backlog ทั้งหมดตอนหน้าต่างเปิด**
  (เปลี่ยนจาก ADR-0003 เดิม → ADR-0004) เฉพาะแถวที่ `created_at` (เวลาของวัน) ตกอยู่ในช่วง
  เวลาเท่านั้นที่ถูก auto-send — แถวที่เกิดนอกช่วงจะค้าง Pending **ตลอดไป** แม้หน้าต่างจะเปิดใน
  รอบถัดไปก็ตาม กรองที่ SQL `WHERE` ของ `had_send_pending()` ตรงๆ (กัน starvation จากแถวเก่า
  บัง `ORDER BY id ASC LIMIT 50`) ต้องสั่งส่งเองทาง "ส่งซ้ำทันที" เท่านั้น — **HAD ตัด Re-queue
  ออกไปเลย** เพราะ Re-queue เดิมไม่แตะ `created_at` จะกลายเป็นปุ่มไม่มีผลกับแถวนอกช่วง
  (Dashboard จะเห็นยอด "รอส่ง" ไต่ขึ้นทุกวันสำหรับแถวนอกช่วงที่ไม่มีใครสั่งส่งเอง — ไม่ใช่บั๊ก)
- **Cron ไม่ถูกแตะ** ยังวิ่งทุก 5 นาที 24 ชม. เหมือนเดิม เพราะการแก้ scheduled task
  ต้องใช้ UAC ซึ่งหน้าเว็บสั่งไม่ได้ — worker กั้นตัวเองแทน (เหตุผลเต็มใน `docs/adr/0004`,
  `docs/adr/0003` เดิม)
...
- ถ้าจะทำโมดูลอื่นตาม: store เป็น module-keyed อยู่แล้ว เพิ่ม key ใน `ALERT_WINDOW_DEFAULTS`
  + ปุ่ม/modal ในหน้านั้น + gate รอบการเรียก `*_send_pending()` ของ worker (ตัดสินใจเองว่าจะเอา
  แบบ per-record `created_at` ตาม ADR-0004 หรือจะ flush ทั้ง backlog ตาม ADR-0003 เดิม)
