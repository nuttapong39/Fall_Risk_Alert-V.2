...
/**
 * sexual_action.php
 * AJAX action handler สำหรับ sexual.php
 *
 * POST action=send  vn=<VN>
 *   → query HOSxP ด้วย VN + lab_items_code
...
if (!defined('SEXUAL_CLIENT_KEY'))
  define('SEXUAL_CLIENT_KEY', defined('MOPH_CLIENT_KEY') ? MOPH_CLIENT_KEY : '');
if (!defined('SEXUAL_SECRET_KEY'))
  define('SEXUAL_SECRET_KEY', defined('MOPH_SECRET_KEY') ? MOPH_SECRET_KEY : '');
...
function sexual_log(array $row, $code, $resp, $err = null): void {
  global $LOG_FILE;
...
try {
  ...
  $stmt->execute([$vn, LAB_CODE_SEXUAL]);
  $row = $stmt->fetch(PDO::FETCH_ASSOC);

  if (!$row) {
    echo json_encode(['ok'=>false, 'msg'=>"ไม่พบข้อมูล VN: {$vn} (lab_items_code=".LAB_CODE_SEXUAL.")"]);
    exit;
  }
  ...
  // ── Build Flex payload ────────────────────────────────────────────────────
  $payload = buildSexualPayload($row);
  $body    = json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);

  if ($body === false) {
    $jsonErr = json_last_error_msg();
    sexual_log($row, 0, null, "JSON_ENCODE_FAIL: $jsonErr");
