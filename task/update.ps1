...
function Set-Status([string]$status, [string]$message) {
  $obj = [ordered]@{ status = $status; message = $message; updatedAt = (Get-Date -Format 's') }
...
try {
  Set-Status 'running' "เริ่มอัปเดต (โฟลเดอร์: $AppDir)"

  # ── 1) สำรองก่อนเสมอ ─────────────────────────────────────────────────────
  Set-Status 'running' 'กำลังสำรองข้อมูล...'
  ...
  Set-Status 'running' "สำรองแล้วที่ $BackupDir"
  ...
  if ($UseGit) {
    Set-Status 'running' 'พบ .git -> git fetch + reset --hard origin/master'
    ...
    Set-Status 'running' 'ไม่พบ .git -> ดาวน์โหลด ZIP จาก GitHub'
    ...
    Set-Status 'running' 'กำลังคัดลอกไฟล์ทับ (ยกเว้น secrets/ และ logs/)'
  ...
  Set-Status 'running' 'กำลังตรวจ/สร้างตาราง DB ที่ขาด (migration)...'
