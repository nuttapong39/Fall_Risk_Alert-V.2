...
/**
 * patient_action.php
 * ตัวรับคำสั่ง Bulk action จากหน้า patient.php
 *   - send_now     ส่ง Flex ไป MOPH Notify ทันที (ทีละ id)
 *   - requeue      รีเซ็ตสถานะให้กลับไปค้างส่งอีกครั้ง
 *   - clear_error  ล้างข้อความ error
 */
require_once __DIR__ . '/config.php';
// require_once __DIR__ . '/auth_guard.php';
require_once __DIR__ . '/covid_lib.php';      // to_utf8(), row_to_utf8(), extract_moph_message_id()
require_once __DIR__ . '/flex_patient.php';   // ไลบรารี Flex (buildPatientPayload) — ต้องมาก่อนฟังก์ชันท้องถิ่น
...
if (!defined('PATIENT_CLIENT_KEY')) {
  define('PATIENT_CLIENT_KEY', defined('MOPH_CLIENT_KEY') ? MOPH_CLIENT_KEY : '');
}
if (!defined('PATIENT_SECRET_KEY')) {
  define('PATIENT_SECRET_KEY', defined('MOPH_SECRET_KEY') ? MOPH_SECRET_KEY : '');
}

$LOG_DIR = __DIR__ . '/logs';
if (!is_dir($LOG_DIR)) @mkdir($LOG_DIR, 0775, true);
$LOG_FILE = $LOG_DIR . '/moph_alert_patient.log';

/* ====== Helpers ====== */
function log_moph_response_patient($row, $code, $resp, $err=null) {
  global $LOG_FILE;
  $line = sprintf(
...
function send_one_now_patient(PDO $db, int $id): array {
  $get = $db->prepare("SELECT * FROM patient_queue WHERE id=:id");
  $get->execute([':id'=>$id]);
  $row = $get->fetch();
  ...
  if ($body === false) {
    ...
    log_moph_response_patient($row, 0, null, "JSON_ENCODE_FAIL: ".$jsonErr);
    $upd = $db->prepare("UPDATE patient_queue SET last_attempt_at=NOW(), attempt=attempt+1, last_error=:e WHERE id=:id");
  ...
  log_moph_response_patient($row, $code, $resp, $err);
  ...
  if (($code>=200 && $code<300) && $looksSuccess) {
    ...
    $upd = $db->prepare("UPDATE patient_queue SET status=1, sent_at=NOW(), last_attempt_at=NOW(), attempt=attempt+1, last_error=NULL, out_ref=:r, line_message_id=:r WHERE id=:id");
...
try {
  if ($action === 'requeue') {
    ...
    $stmt = $dbcon->prepare("UPDATE patient_queue
                                SET status=0, attempt=0, last_attempt_at=NULL,
                                    last_error=NULL, out_ref=NULL, line_message_id=NULL
                              WHERE id IN ($place)");
    $stmt->execute($ids);
    header('Location: patient.php?msg=requeued&affected='.$stmt->rowCount()); exit;

  } elseif ($action === 'clear_error') {
    $place = implode(',', array_fill(0, count($ids), '?'));
    $stmt = $dbcon->prepare("UPDATE patient_queue SET last_error=NULL WHERE id IN ($place)");
    $stmt->execute($ids);
    header('Location: patient.php?msg=cleared&affected='.$stmt->rowCount()); exit;

  } elseif ($action === 'send_now') {
    $ok=0; $fail=0;
    foreach ($ids as $id) {
      [$o,$r,$e] = send_one_now_patient($dbcon, (int)$id);
      if ($o) $ok++; else $fail++;
    }
    header('Location: patient.php?msg=sendnow&ok='.$ok.'&fail='.$fail); exit;

  } else {
    header('Location: patient.php?msg=bad_action'); exit;
  }
} catch (Throwable $e) {
  header('Location: patient.php?msg=err&detail='.urlencode($e->getMessage()));
}
