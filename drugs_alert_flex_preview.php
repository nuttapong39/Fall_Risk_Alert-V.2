<?php
/**
 * drugs_alert_flex_preview.php — ดูตัวอย่างการ์ด Flex ของ Drug Alert (ยาเฝ้าระวัง)
 *
 * ใช้หน้าตาการ์ดชุดเดียวกับ flex_editor.php (ดีไซน์ปัจจุบัน) ไม่ใช่แบบเก่าของ
 * fracture/pharm_lab — สี/หัวเรื่อง/footer อ่านจาก flex_theme('drugs_alert') ทุกจุด
 * จึงเปลี่ยนตามที่แก้ใน flex_editor ทันทีโดยไม่ต้องแตะไฟล์นี้ (แพทเทิร์นเดียวกับ
 * lab_hemato_flex_preview.php — มี auth_guard จริง + validator สี ไม่หลุด PHI แบบไฟล์เก่า)
 *
...
try {
  if (!empty($_GET['id']) && ctype_digit((string)$_GET['id'])) {
    $st = $dbcon->prepare("SELECT * FROM drugs_alert_queue WHERE id=?");
    $st->execute([(int)$_GET['id']]);
    $row = $st->fetch(PDO::FETCH_ASSOC) ?: null;
  }
  if (!$row) {
    $row = $dbcon->query("SELECT * FROM drugs_alert_queue ORDER BY id DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC) ?: null;
  }
} catch (Throwable $ex) { $row = null; }

$isSample = ($row === null);
if ($isSample) {
  $row = ['hn'=>'0012345','fullname'=>'นายตัวอย่าง ทดสอบ','age'=>60,'sex'=>'1',
          'icode'=>'0000000','drug_name'=>'ยาตัวอย่าง','strength'=>'500','units'=>'mg',
          'vstdate'=>date('Y-m-d'),'qty'=>'30','sum_price'=>'120.00',
          'doctor'=>'','hometel'=>'08X-XXX-XXXX','address'=>'-'];
}

$t = flex_theme('drugs_alert');
$g = flex_theme_global();
$L = $g['labels'];

/* payload จริงที่จะถูกส่งออก — ใช้ตรวจสีและดู JSON ได้ในหน้าเดียวกัน */
$payload = buildDrugsAlertPayload($row);
...
$qty  = trim(($row['strength'] ?? '') . ' ' . ($row['units'] ?? ''));

$PAGE_TITLE = 'ตัวอย่าง Flex — Drug Alert';
...
.dap-json{background:#0f172a;color:#e2e8f0;border-radius:12px;padding:14px;font-size:11.5px;max-height:460px;overflow:auto}
</style>
HTML;
...
<div class="page-header">
  ...
  <div class="d-flex gap-2">
    <a href="flex_editor.php?m=drugs_alert" class="btn btn-outline-primary btn-sm">
      <span class="msi me-1">palette</span>แก้สี/ข้อความการ์ด
    </a>
    <a href="drugs_alert.php" class="btn btn-outline-secondary btn-sm">
    ...
    <div class="fe">
      <div class="fe-card">
        ...
        <div class="bd">
          ...
          <div class="lb">รายการยาที่เฝ้าระวัง</div>
          <div class="icd">
            <span class="l" style="color:<?= $e($t['accent']) ?>">ชื่อยา</span>
            <span class="r" style="font-size:15px"><?= $e($row['drug_name'] ?: '-') ?></span>
          </div>
          <div class="kv"><span class="k">รหัสยา (icode)</span><span class="v"><?= $e($row['icode']) ?></span></div>
          <div class="kv"><span class="k">ความแรง/หน่วย</span><span class="v"><?= $e($qty !== '' ? $qty : '-') ?></span></div>
      ...
      <div class="text-center text-muted mt-2" style="font-size:.78rem">
        พรีวิว — สี/หัวเรื่อง/footer อ่านจาก <code>flex_theme('drugs_alert')</code> ตัวเดียวกับที่ส่งจริง
      </div>
    </div>
  </div>
