<?php
/**
 * scrub_queue_action.php — Action handler สำหรับ scrubtyphus.php
 *
 * POST action=import_hosxp   start end              → Sync HOSxP → scrub_queue (JSON)
...
if (!defined('SCRUB_CLIENT_KEY'))    define('SCRUB_CLIENT_KEY',    defined('MOPH_CLIENT_KEY') ? MOPH_CLIENT_KEY : '');
if (!defined('SCRUB_SECRET_KEY'))    define('SCRUB_SECRET_KEY',    defined('MOPH_SECRET_KEY') ? MOPH_SECRET_KEY : '');

// ── Logging ───────────────────────────────────────────────────────────────────
$LOG_DIR  = __DIR__ . '/logs';
if (!is_dir($LOG_DIR)) @mkdir($LOG_DIR, 0775, true);
$LOG_FILE = $LOG_DIR . '/moph_alert_scrub.log';
...
function dq_send_row(array $qRow): array {
  ...
  $payload = buildDiseasePayload($payloadRow, 'scrub');
  ...
  curl_setopt_array($ch, [
    ...
    CURLOPT_HTTPHEADER     => [
      'client-key: ' . SCRUB_CLIENT_KEY,
      'secret-key: ' . SCRUB_SECRET_KEY,
...
if ($action === 'import_hosxp') {
  ...
  $chkStmt = $dbcon->prepare("SELECT id FROM scrub_queue WHERE vn=?");
...
if ($action === 'send_queue_item') {
  ...
  $qRow = $dbcon->query("SELECT * FROM scrub_queue WHERE id=$id LIMIT 1")->fetch(PDO::FETCH_ASSOC);
  ...
  if ($result['ok']) {
    $dbcon->prepare(
      "UPDATE scrub_queue SET status=1, sent_at=NOW(), last_attempt_at=NOW(),
    ...
    $dbcon->prepare(
      "UPDATE scrub_queue SET last_attempt_at=NOW(), attempt=attempt+1, last_error=:e WHERE id=:id"
...
$token = $_POST['token'] ?? '';
if ($token !== SCRUB_Q_UI_TOKEN) {
  header('Location: scrubtyphus.php?msg=bad_action');
  exit;
}
...
if (empty($ids) && $action !== 'import_hosxp') {
  header('Location: scrubtyphus.php?msg=no_ids');
  exit;
}
...
/* ── send_now ──────────────────────────────────────────────────────────────── */
if ($action === 'send_now') {
  $rows   = $dbcon->prepare("SELECT * FROM scrub_queue WHERE id IN ($placeholders)");
  $rows->execute($ids);
  ...
  header("Location: scrubtyphus.php?msg=sendnow&ok={$sentOk}&fail={$sentFail}");
  exit;
}
...
if ($action === 'requeue') {
  $stmt = $dbcon->prepare(
    "UPDATE scrub_queue SET status=0, attempt=0, last_error=NULL, last_attempt_at=NULL WHERE id IN ($placeholders)"
  );
  $stmt->execute($ids);
  $aff = $stmt->rowCount();
  header("Location: scrubtyphus.php?msg=requeued&affected={$aff}");
  exit;
}
...
if ($action === 'clear_error') {
  $stmt = $dbcon->prepare(
    "UPDATE scrub_queue SET last_error=NULL WHERE id IN ($placeholders)"
  );
  $stmt->execute($ids);
  $aff = $stmt->rowCount();
  header("Location: scrubtyphus.php?msg=cleared&affected={$aff}");
  exit;
}
