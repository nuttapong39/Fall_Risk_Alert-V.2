<?php
/**
 * lepto_ingest.php — CLI worker สำหรับ Task Scheduler
 * ─────────────────────────────────────────────────────────────────────────────
 * ต้องรันผ่าน PHP CLI เท่านั้น (ไม่ใช่ผ่านเว็บ)
 *
 * Usage:
 *   php lepto_ingest.php                        — ดึง + ส่ง 7 วันล่าสุด (default)
 *   php lepto_ingest.php dryrun                 — แสดงผลเฉพาะ ไม่ส่งจริง
 *   php lepto_ingest.php start 2025-06-01       — backfill ตั้งแต่วันที่ระบุ
 *   php lepto_ingest.php start 2025-06-01 end 2025-12-31
 */

/* ── CLI guard ─────────────────────────────────────────────────── */
if (PHP_SAPI !== 'cli') {
  http_response_code(403);
  exit("lepto_ingest.php must be run from CLI only.\n");
}
...
$LOG_FILE = $LOG_DIR . '/lepto_task_run.log';
$MOPH_LOG = $LOG_DIR . '/moph_alert_lepto.log';
...
/* ═══ START ══════════════════════════════════════════════════════ */
dng_log('=== lepto_ingest START' . ($isDry ? ' [DRYRUN]' : '') . ' ===');
dng_log("ช่วง: {$start} ถึง {$end}  lab_items_code=" . LEPTO_LAB_CODE);

/* ═══ STEP 1: Query HOSxP ════════════════════════════════════════ */
try {
  require_once __DIR__ . '/sources/lepto_source.php';
  $hosxpRows = lepto_source_rows($start, $end, LEPTO_LAB_CODE);
...
if ($isDry) {
  ...
  dng_log('=== lepto_ingest DONE [DRYRUN] ===');
  exit(0);
}
...
foreach ($pending as $qRow) {
  ...
  if (!$err && ($code >= 200 && $code < 300) && $ok) {
    ...
    telegram_mirror('lepto', '🐀 แจ้งเตือนผู้ป่วยเลปโตสไปโรสิส', $qRow);
