<?php
/**
 * dengue_queue_action.php — Action handler สำหรับ dengue_queue_ui.php
 *
 * POST action=import_hosxp   start end              → Sync HOSxP → dengue_queue (JSON)
...
$LOG_FILE = $LOG_DIR . '/moph_alert_dengue.log';
...
if ($action === 'send_queue_item') {
  ...
  $qRow = $dbcon->query("SELECT * FROM dengue_queue WHERE id=$id LIMIT 1")->fetch(PDO::FETCH_ASSOC);
...
$token = $_POST['token'] ?? '';
if ($token !== DENGUE_Q_UI_TOKEN) {
  header('Location: dengue_queue_ui.php?msg=bad_action');
  exit;
}
...
if (empty($ids) && $action !== 'import_hosxp') {
  header('Location: dengue_queue_ui.php?msg=no_ids');
  exit;
}
...
/* ── send_now ──────────────────────────────────────────────────────────────── */
if ($action === 'send_now') {
  $rows   = $dbcon->prepare("SELECT * FROM dengue_queue WHERE id IN ($placeholders)");
  $rows->execute($ids);
  ...
  header("Location: dengue_queue_ui.php?msg=sendnow&ok={$sentOk}&fail={$sentFail}");
  exit;
}
...
if ($action === 'requeue') {
  $stmt = $dbcon->prepare(
    "UPDATE dengue_queue SET status=0, attempt=0, last_error=NULL, last_attempt_at=NULL WHERE id IN ($placeholders)"
  );
  $stmt->execute($ids);
  $aff = $stmt->rowCount();
  header("Location: dengue_queue_ui.php?msg=requeued&affected={$aff}");
  exit;
}
...
if ($action === 'clear_error') {
  $stmt = $dbcon->prepare(
    "UPDATE dengue_queue SET last_error=NULL WHERE id IN ($placeholders)"
  );
  $stmt->execute($ids);
  $aff = $stmt->rowCount();
  header("Location: dengue_queue_ui.php?msg=cleared&affected={$aff}");
  exit;
}
